...
- A icosahedron boundary token around the PCB (or around the host↔PCB link, whichever reads better).
- Host computer on one side; PCB on the other; a cable between them.
- **Information** tethers (cyan) along the data link: commands → PCB, telemetry/events → host.
- **Power** tether (amber) entering the PCB from a supply connector/rail.
- Keep this panel uncluttered: 2–4 tethers total.
