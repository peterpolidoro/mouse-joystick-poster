# P04 — Top-level system boundary: instrument → artifacts

## 0) Narrative role (why this panel exists)
This is the first “big zoom” into the physical world: the full instrument stack (mouse rig + electronics + host computer) as one system boundary.
...
- This file: `03_PANELS/P04_system_boundary/context.md` (the one you’re reading)

### Panel‑specific REQUIRED uploads
- [ ] At least ONE reference image of the overall joystick rig setup OR a CAD/render of the apparatus.

### Panel‑specific OPTIONAL uploads (helps accuracy)
- [ ] A photo of the host computer + cabling.
- [ ] A photo/render of the PCB.
- [ ] A photo of the power supply / battery used.

### Text info to paste into the chat (if you want accuracy)
- Top-level system name (optional).
- Power source description (e.g., ‘12V supply’).

---

## 5) Output spec (so it drops into the template cleanly)
- **Panel physical size in template:** ~27.08×17.60 inches
- **Aspect ratio (approx):** 1.538 (W/H)
- **Suggested render size:** 4096×2662 px (or the **largest** your image tool allows at this aspect ratio)
